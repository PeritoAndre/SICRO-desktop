...
import { findTemplate, TOOLBAR_TEMPLATES, type TemplateId } from "../engine";
...
const CATEGORIES: CategoryDef[] = [
  ...
  },
  {
    ...
    subtools: [
      ...
      { key: "vehicle_van", label: "Van", icon: Bus },
      { key: "vehicle_onibus", label: "Ônibus", icon: Bus },
      { key: "vehicle_truck", label: "Caminhão leve", icon: Bus },
      { key: "vehicle_caminhao_pesado", label: "Caminhão pesado", icon: Bus },
      { key: "vehicle_carreta", label: "Carreta", icon: Bus },
...
export function Toolbar({
  ...
  useEffect(() => {
    if (!openPopover && !templatesOpen) return;
  ...
  return (
    ...
    >
      ...
      {/* Modelos de via (apenas templates Road Engine Pro) */}
      <div className={styles.group}>
        <div className={styles.groupTitle}>
          <Workflow size={11} /> Modelos
        </div>
        <button
          type="button"
          ...
          title="Inserir modelo de via pronto (Road Engine Pro)"
        >
          <Workflow size={12} /> Modelos…
        </button>
        {templatesOpen && (
          <div className={styles.dropdown}>
            {TOOLBAR_TEMPLATES.map((id) => {
