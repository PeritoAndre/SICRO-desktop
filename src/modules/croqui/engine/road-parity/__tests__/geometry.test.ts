...
describe("road-parity / polylineLength", () => {
  it("reta de 100m mede 100m", () => {
    const road = makeParityRoad(0, 0, 100, 0);
    const samples = sampleCubicBezier(road, 32);
    expect(polylineLength(samples)).toBeCloseTo(100, 1);
  });

  it("curva em S é mais longa que a reta entre endpoints", () => {
    const road = makeParityRoadBezier(0, 0, 30, -50, 70, 50, 100, 0);
    const samples = sampleCubicBezier(road, 64);
    expect(polylineLength(samples)).toBeGreaterThan(100);
  });
});
