...
fn inline_to_runs(node: &Value, base_size: Option<usize>, bold_default: bool) -> Vec<Run> {
    match type_of(node) {
        ...
        // Pílula de campo automático (`{numero_laudo}` etc.). O walker do
        // backend não tem acesso ao resolver (occurrence/metadata vivem no
        // front), então emitimos o placeholder textual `{<key>}` — o usuário
        // que abre o .docx vê a chave igual no Word, podendo editar. O export
        // PDF/HTML segue resolvendo via runtime do front. Sem este branch, o
        // walker SILENCIOSAMENTE perdia toda pílula (caía no `_ => Vec::new()`).
        "fieldPlaceholder" => {
            let key = node
                .get("attrs")
