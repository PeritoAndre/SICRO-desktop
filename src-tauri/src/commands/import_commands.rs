...
use crate::database::repositories::{import_repo, media_asset_repo};
use crate::error::{Result, SicroError};
use crate::importer::{run_import, ImportRegistry};
use crate::models::{Import, ImportReport, ImportResult, ImportSicroappInput, MediaAsset};
...
/// List the media assets registered for a workspace's active occurrence.
/// Used by the dossier "fotos" tab to display the gallery.
#[tauri::command]
pub async fn list_workspace_photos(workspace_path: String) -> Result<Vec<MediaAsset>> {
    let ws = PathBuf::from(&workspace_path);
