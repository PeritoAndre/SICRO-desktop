...
pub async fn open_sigdocs_cover(
    ...
    // L — `always_on_top: true` é crítico aqui. Sem ele, qualquer
    // clique na main window do SICRO faz o OS trazer a main pro
    // foreground e o cover do SIGDOC "afunda" no z-order, ficando
    // ESCONDIDO atrás da main window (com fundo preto onde o cover
    // estava). Como o cover é tecnicamente uma window secundária do
    // SO mas LOGICAMENTE faz parte da main, ele precisa flutuar
    // sempre por cima.
    //
...
/// de Resize/Moved/Focused/CloseRequested na main window pra
/// reposicionar / re-elevar / fechar o cover quando necessário.
pub fn install_cover_resize_listener(app: &AppHandle) {
    ...
    main_window.on_window_event(move |event| {
        match event {
            ...
            // L — Quando a main ganha foco (user clicou no SICRO),
            // garante que o cover continua VISÍVEL por cima dela. Sem
            // isso, mesmo com always_on_top, o Windows às vezes traz
            // a main pra frente e o cover fica "embaixo" visualmente
            // (já vi acontecer no Windows 10/11 com multi-monitor).
            //
            // Também trata o caso de minimizar/restaurar: quando a
            // main minimiza, escondemos o cover; quando restaura,
            // mostramos de novo.
            WindowEvent::Focused(is_focused) => {
                ...
                if *is_focused {
                    // Re-eleva o cover ao foreground sem roubar foco
                    // da main.
                    let _ = cover.set_always_on_top(true);
                    // `show()` é idempotente; garante que o cover
                    // está visível.
                    let _ = cover.show();
                }
            }
